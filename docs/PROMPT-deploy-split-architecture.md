...
## ملحق — تصحيحات بعد النشر الأول (2026-08-31)

نُشر هذا البرومت ونُفِّذ بنجاح، لكن الواقع خالف افتراضاته وكشف ثلاث علل في
المستودع نفسه. **أُصلحت الثلاث في الأصل**، فمن يقرأ هذا الملف الآن لا يحتاج
حلولاً التفافية.

### ما كان خطأ في البرومت

- **افترض Node عارياً تحت systemd.** الواقع Docker Compose. مجلد العمل وأمر
  التشغيل في الخطوة 6 لا ينطبقان؛ استخدم `docker compose up -d --build`.
- **افترض المنفذ 3000.** كان محجوزاً لمشروع آخر، والنشر الفعلي على 3100.
  المنفذ الداخلي لا يهم ما دام nginx يعرفه — لا تفترضه.
- **الخطوة 2 (نقل `.env` و`uploads/`) لا تنطبق على Docker.** الرفوعات في
  volume مسمّى، و`.env` يقرأه compose من جذر المشروع. نقلهما يكسر النشر.
- **الخطوة 4 اقترحت `npm test` على السيرفر.** هذا خطر: الاختبار ينفّذ
  `migrate()` و`seed()` ويكتب صفوفاً في أي قاعدة يُوجَّه إليها. أصبح الاختبار
  الآن **يرفض العمل** عند `NODE_ENV=production` (يُتجاوَز عمداً بـ
  `ALLOW_TESTS_ON_PRODUCTION=true`). شغّله على حاوية MySQL مؤقتة.

### ما أُصلح في المستودع

- `docker-compose.yml` كان لا يمرّر `APP_*` إلى الحاوية، فتبقى
  `/api/app/version` تعلن `null` مهما ضُبط `.env`. صار يمرّرها.
- `web/config.js` كان يشحن `apiBase: 'http://localhost:3000'` مثبَّتاً فيكسر
  الإنتاج. صار الافتراضي **نفس الأصل** (`''`)، و`web/serve.js` يمرّر
  `/api` و`/uploads` و`/downloads` و`/socket.io` في التطوير — فتتطابق البيئتان
  ولا يحتاج الملف تعديلاً عند النشر.
- `docker-compose.yml` صار يربط `./server/downloads` كـbind mount، ويربط
  المنافذ بـ`127.0.0.1` افتراضياً (`BIND_HOST` للتجاوز).

**عند أول `git pull` بعد هذه التصحيحات:** أي تعديل يدوي على `web/config.js`
سيتعارض — احذف التعديل المحلي واقبل نسخة المستودع. وتحقّق هل بقي
`docker-compose.override.yml` يحمل `APP_*` مكرّرة؛ التكرار غير ضار لكن الأصل
صار كافياً.

### مزلق nginx يستحق التكرار

`location /api` يبتلع `/api.js` فيرجع الملف الثابت 404 وتتعطل الواجهة كلها.
الصحيح المطابقة على حدود المسار:
...
ونفس المعالجة لـ`/uploads` و`/downloads` و`/socket.io`.
