...
### ٩‑ب. 🛑 تحقّق من App Links على جهاز حقيقي (issue #44) — لا اختصار

```bash
curl -s https://munasbat.ktra-pro.tech/.well-known/assetlinks.json
```

يجب أن يعيد مصفوفة JSON صالحة ببصمتَي SHA-256 (لا بصمة واحدة — أندرويد 7–12
يتحقق بالـdebug، و13+ بالإصدار). ثم **على هاتف حقيقي مثبَّت عليه هذا الإصدار**:
افتح رابط `https://munasbat.ktra-pro.tech/e/<رقم مناسبة حقيقي>` من رسالة واتساب
أو ملاحظة (لا من شريط عنوان المتصفح — بعض المتصفحات لا تُشغّل تحقق App Links من
هناك)، وتأكّد أنه **يفتح التطبيق مباشرة على شاشة تفاصيل تلك المناسبة، بلا مربّع
اختيار تطبيق**.

⚠️ **هذا الفحص لا بديل آلي عنه ولا يظهر خطأ عند فشله.** إن كان التحقق الرقمي
(Digital Asset Links) لم يكتمل — بصمة ناقصة، أو ملف لم يُنشر، أو تعديل لم
يُرفع بعد — أندرويد لا يعرض رسالة ولا سجلاً؛ الرابط **يفتح المتصفح بصمت** وكأن
لا تطبيق مثبَّتاً أصلاً. لا طريقة لاكتشاف هذا إلا التجربة الفعلية على جهاز.
