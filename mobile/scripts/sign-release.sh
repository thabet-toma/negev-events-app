...
#
# 🔑 دوران المفتاح (issue #44): web/.well-known/assetlinks.json يحمل بصمتَي
# SHA-256 لشهادتي التوقيع (الإصدار والـdebug القديمة عبر النسب أعلاه) — أي
# إصدار يدوّر أياً منهما (نسب جديد، أو مفتاح إصدار مختلف) **يجب** أن يُحدَّث
# ذلك الملف ببصمة `apksigner verify --print-certs` الجديدة **قبل** نشر ذلك
# الإصدار، وإلا يفشل App Links بصمت على كل جهاز يتحقق بالبصمة القديمة.
set -euo pipefail
