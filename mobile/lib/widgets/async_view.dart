...
/// فتح رابط خارج التطبيق (البث، قناة البث، يوتيوب) — تعريف واحد للفشل
/// ورسالته. `launchUrl` يفشل بطريقتين لا واحدة: يردّ false حين لا يجد من
/// يفتح الرابط، ويرمي حين يرفض النظام الطلب أصلاً؛ القيمة المرتدّة وحدها
/// تترك الرمية تفلت من معالج الضغط فتصير النقرة ميتة بلا أي رسالة.
Future<void> openExternalLink(
  ...
  if (!opened && context.mounted) {
    showMessage(context, failureMessage, isError: true);
  }
}
