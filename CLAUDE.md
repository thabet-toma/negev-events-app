...
**البنية صارت تدعم أنواع المناسبات، لكن مسار النشر لم يُحدَّث بعد.** جدول `occasion_types`
(+ `occasion_type_fields` + `occasion_type_reactions`) يحمل خمسة أنواع مزروعة —
عرس، عزا، خطوبة، نجاح، حج وعمرة — كبيانات وقت تشغيل يديرها `super_admin` من
`GET/POST/PATCH/DELETE /api/admin/occasion-types` بلا نشر ولا ترحيل لكل نوع جديد.
`events.occasion_type_id` (NULL قابل، FK `ON DELETE RESTRICT`) يربط كل مناسبة بنوعها،
و`event_honorees` جدول ١..N لأصحاب المناسبة (بديل تعميم `groom_name` الفردي).
الصفوف القديمة كلها رُحِّلت إلى نوع «عرس» تلقائياً (`dataMigrations.js`).

**لكن**: `POST /api/events` (مسار النشر)، البحث، الترقيم، التبريكات، والواجهات
(`web/` و`mobile/`) **لم تُحدَّث بعد** — لا تزال تفترض عرساً واحداً بحقول عرس ثابتة
(`groom_name`، `youth_party_date`، `dinner_time`، تبريكة «مبارك الفرح» افتراضية،
تفاعلات `coffee` `horse` `fireworks` `rose` `hand` بلا فحص نوع). عزاء منشور اليوم عبر
هذا المسار سيُحفَظ ببيانات عرس، لا كعزاء. إن كانت مهمتك تلمس النشر أو الواجهات فتحقّق
أولاً من حالة هذه الخطوات في issue #20 قبل الافتراض أنها منتهية.
