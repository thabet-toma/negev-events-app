...
**لا يُعتمد أي تغيير كتبه منفِّذ آخر (وكيل مفوَّض، جلسة أخرى، أي مصدر) قبل قراءة `git diff`
النهائي كاملاً، سطراً بسطر، بعينك.** هذه قراءة الكود نفسه، لا بدائله:

- **البوّابات الخضراء ليست قراءة.** `npm test` لا يرى تخطيطاً (لا محرّك layout في jsdom)، ولا
  صنف CSS مكرَّراً يتغلّب على قاعدة قديمة، ولا تعليقاً كذّبه التغيير، ولا توثيقاً صار خطأً.
...
أو `CLAUDE.md` صار يصف سلوكاً استُبدل · تأكيد اختبار لا يستطيع السقوط للسبب الذي يحمله اسمه.
