...
- كل مسارات `/admin` خلف `requireAdmin` أو أشدّ، ولا مفاتيح تجاوز. **والدوران لم يعودا متساويين:** `/admin/occasion-types/*` و `/admin/villages/*` و `/admin/events/:id/promote-village` و `/admin/service-categories/*` و `/admin/stories/*` و `/admin/live/*` و `/admin/admins/*` و `/admin/analytics/*` و `GET /admin/users` خلف `requireSuperAdmin` حصراً، و`PATCH /admin/users/:id/role` (ترقية/إلغاء صلاحية) يفرض نفس القيد لكن داخل معالِجه مباشرة لا عبر `requireSuperAdmin` — فيردّ 404 لا 403 على أدمن غير سوبر، على نمط `adminScope.service.js` (كل راوتر يحمل حارسه الخاص؛ حارس `admin.routes.js` لا يحمي راوترات أخرى تشارك بادئة `/admin`). **`POST /admin/broadcast` خلف `requireAdmin` فقط منذ #85 (خطوة 22-24)** — أدمن بلدة يبثّ لبلداته هو، والنطاق يُبنى بكامله داخل `adminScope.resolveBroadcastTowns` لا في الراوتر؛ أدمن بلا بلدات مُسنَدة يُرفض صراحة بلا كتابة أي صفّ
...
- `events.village_id` و`events.requested_village_name` («قريتي غير موجودة») غير NULL **فقط** حين `town = 'القرى والتجمعات'` (‏`VILLAGES_TOWN`)، **ولا يجتمعان** — قاعدة يفرضها الكود لا القاعدة، والقرية تُورِّث إحداثياتها للمناسبة عند النشر **وعند التعديل**. نشر جديد تحت هذا البند يحمل أحدهما إلزامياً؛ اسم مكتوب يطابق قرية نشِطة يُربط بها فوراً، وغيره ينتظر `POST /admin/events/:id/promote-village` (سوبر أدمن) الذي ينشئ القرية أو يعيد تنشيطها ويربط كل مناسبة طلبت الاسم نفسه
