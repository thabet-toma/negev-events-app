...
القادم ونفس ترشيح النسخة. **التبويبات في الواجهة، والتبريكات ومراجعتها (الكتابة)، والإعلان
والتذكير، والستوريات، والواجهات (`web/` و`mobile/`) لا تزال غير محدَّثة** — لا تزال تفترض
عرساً واحداً بحقول عرس ثابتة (تبريكة «مبارك الفرح» افتراضية، تفاعلات `coffee`
`horse` `fireworks` `rose` `hand` بلا فحص نوع في هذه الطبقات). إن كانت مهمتك تلمس هذه
الطبقات أو الواجهات فتحقّق أولاً من حالتها في issue #20 قبل الافتراض أنها منتهية.
