...
`server/src/routes/` (كل المسارات، سبعة ملفات) · `server/.env.example` (أسماء متغيرات البيئة).
...
| أيقونة أو شعار | العلامة (بيت الشَّعَر تحت أضواء العرس داخل طوق) معرَّفة مرة واحدة في `buildMarkParts` بـ`server/src/utils/brandMark.js` — لا مكان آخر. `server/scripts/brand-icons.js` يستهلكها ويولّد منها ٢٥ ملف PNG + `icon.svg` في `web/icons/` و`mobile/`؛ `server/src/routes/share.routes.js` و`server/src/services/shareCard.service.js` يستهلكانها مباشرة أيضاً. طبقتان بعلَم `detail`: `'icon'` لكل ما يُعرض أيقونةَ تطبيق، و`'full'` لما يُعرض كبيراً فعلاً. لا تحرّر PNG ولا SVG بيدك؛ عدّل الهندسة في `brandMark.js` وأعد تشغيل `node scripts/brand-icons.js` |
...
جدول `occasion_types` (+ `occasion_type_fields` + `occasion_type_reactions`) يحمل خمسة
أنواع مزروعة — عرس، عزا، خطوبة، نجاح، حج وعمرة — كبيانات وقت تشغيل يديرها `super_admin`
من `GET/POST/PATCH/DELETE /api/admin/occasion-types` بلا نشر ولا ترحيل لكل نوع جديد.
`events.occasion_type_id` (NULL قابل، FK `ON DELETE RESTRICT`) يربط كل مناسبة بنوعها،
و`event_honorees` جدول ١..N لأصحاب المناسبة (بديل تعميم `groom_name` الفردي). الصفوف
القديمة كلها رُحِّلت إلى نوع «عرس» تلقائياً (`dataMigrations.js`).

**مسار النشر (`POST /api/events`) صار يعرف النوع فعلاً.** يطلب `occasion_type_id`
إلزامياً من نوع نشِط، و`honorees[]` (اسم واحد على الأقل، بلا سقف عملي يذكر)، ويتحقّق من
كل حقل آخر حسب إعداد `occasion_type_fields` لهذا النوع تحديداً — حقل موسوم إجبارياً
وفارغ يُرفض برسالة تحمل تسميته في هذا النوع تحديداً («المتوفَّى» لا «العريس»)، وحقل غير
ظاهر في النوع يُتجاهَل بصمت إن أُرسل. **صار خلف `authenticate`** (كان عاماً) — الملكية
تُبنى من النشر نفسه (`events.created_by`)، والصفوف القديمة تبقى بلا مالك حتى ينقلها
الأدمن يدوياً عبر `PATCH /api/admin/events/:id/owner` (لا استدلال قرابة آلي). التعديل
عبر `PATCH /api/events/:id` (المالك أو الأدمن) يصنَّف تجميلياً (يبقى `approved`) أو
حرِجاً على التاريخ/المكان (يعود `pending`) — التصنيف بالكود لا بالقاعدة. البحث العام
يمسك أيضاً أي اسم داخل `event_honorees`، لا `groom_name` وحده.

**البحث لا يزال عاماً وليس ما تبقّى من الخطوة.** `GET /api/events` صار قادماً-أولاً
(`COALESCE(event_end_date, event_date) >= CURDATE()`)، مرقّماً (`?page=` `?limit=`، افتراضي
٣٠ وسقف ١٠٠)، مفلتراً حسب النوع صراحةً (`?occasion_type_id=`) ومرشَّحاً حسب ترويسة
`X-App-Version` — طلب بلا الترويسة (كل APK منشور اليوم) لا يرى إلا نوع المناسبة الأول
بالترتيب (`position`)، و`GET /api/events/:id` لنوع آخر يعيد 404. كل صف يحمل أيضاً
`congratulations_count` و`latest_congratulation`، محسوبَين باستعلام نافذة واحد ومُخفيَين معاً
حين يكون `show_congratulations_count` مطفأً على النوع. `GET /api/map/events` يخضع لنفس حدّ
القادم ونفس ترشيح النسخة. **فلتر تغذية `web/` والكرت نفسه أُعيد بناؤهما (#85 دفعة 6أ)** —
رقاقتا مكان/نوع تفتحان ورقة بحث متعددة الاختيار بدل شريطين ثابتين، والكرت صندوق ٤:٥ بالصورة
كاملة. **الإعلان والتذكير، والستوريات، ولوحة الإدارة (`web/admin.js`)، وتطبيق الموبايل
بالكامل لا تزال غير محدَّثة** — لا تزال تفترض عرساً واحداً بحقول عرس ثابتة، تفاعلات `coffee`
`horse` `fireworks` `rose` `hand` بلا فحص نوع في هذه الطبقات. إن كانت مهمتك تلمس هذه الطبقات
فتحقّق أولاً من حالتها في issue #20 قبل الافتراض أنها منتهية.

**التبريكات/التعازي على الخادم صارت تعرف النوع فعلاً (خطوة 5).**
`POST /api/events/:id/congratulate` صار خلف `authenticate` — الهوية تُبنى من الحساب
(`sender_name` منه لا من الجسم، إلا تجاوزاً من الإدارة)، ومحدود بمعدّل لكل مستخدم. الحالة
الابتدائية (`pending` أو `approved`) من علَم `premoderate_messages` على النوع لا من اسمه —
عزاء اليوم وحده. الشارة من `default_badge_title` للنوع لا نص ثابت بالكود؛ نوع بلا شارة
افتراضية لا يُحقَن بأي بديل («مبارك الفرح» ولا «صديق العريس» بأي مسار). رسالة `pending` لا
تُبثّ لحظياً ولا يراها إلا مُرسِلها. المالك أو الإدارة يراجعان طابور مناسبتهما عبر
`GET/PATCH /api/events/:id/congratulations[/:cid]` بنفس منطق `assertCanManageEvent` القائم،
ويحذفان في كل الأنواع عبر `DELETE` — حقّ نابع من الملكية لا امتياز إداري. الإبلاغ
(`POST .../congratulations/:cid/report`، صف واحد لكل مستخدم) يُخفي تلقائياً فوق عتبة
`CONGRATULATION_REPORT_THRESHOLD` في `constants.js`، إلا لرسالة اعتمدها بشرٌ صراحةً
(`moderated_by` مضبوط) — الإبلاغ بديل عن مراجعة بشرية، لا نقض لمراجعة وقعت فعلاً.
**الواجهات (`web/` و`mobile/`) لا تزال غير محدَّثة لأيٍّ من هذا.**

المكان الآخر الذي يقبل نوع مناسبة حرّاً هو `nokoot_ledger.occasion_type` (نص،
افتراضيه `'عرس'`) — منفصل تماماً عن `occasion_types` ولم يتغيّر.
...
- كل مسارات `/admin` خلف `requireAdmin` أو أشدّ، ولا مفاتيح تجاوز. **والدوران لم يعودا متساويين:** `/admin/occasion-types/*` و `/admin/villages/*` و `/admin/service-categories/*` و `/admin/stories/*` و `/admin/admins/*` و `GET /admin/users` خلف `requireSuperAdmin` حصراً، و`PATCH /admin/users/:id/role` (ترقية/إلغاء صلاحية) يفرض نفس القيد لكن داخل معالِجه مباشرة لا عبر `requireSuperAdmin` — فيردّ 404 لا 403 على أدمن غير سوبر، على نمط `adminScope.service.js` (كل راوتر يحمل حارسه الخاص؛ حارس `admin.routes.js` لا يحمي راوترات أخرى تشارك بادئة `/admin`). **`POST /admin/broadcast` خلف `requireAdmin` فقط منذ #85 (خطوة 22-24)** — أدمن بلدة يبثّ لبلداته هو، والنطاق يُبنى بكامله داخل `adminScope.resolveBroadcastTowns` لا في الراوتر؛ أدمن بلا بلدات مُسنَدة يُرفض صراحة بلا كتابة أي صفّ
...
- **البلدات ثابتة بالكود في `server/src/constants.js`.** `mobile/lib/config.dart` ينسخها حرفياً (قاعدة الموبايل أدناه). `web/app.js` (فلتر تغذية المناسبات) يجلبها من `GET /api/towns` بدل نسخها منذ #85 دفعة 6أ؛ `web/admin.js` لا يزال يحمل مصفوفة البلدات التسع ثابتة بالكود — تناقض حقيقي بين نصفَي العميل الواحد، تحلّه دفعة لاحقة من #85 (لا تُصلحه بمهمة عرضية). **القرى وفئات الخدمات بيانات وقت تشغيل ولا تُكرَّر في أي عميل** — تُجلب من `GET /api/towns` و `GET /api/services/categories`. نسخها إلى `mobile/lib/config.dart` أو أي عميل جديد يعيد المشكلة التي وُجدت القرى لحلّها
...
### المخطط
- `migrate.js` يطبّق `schema.sql` بعبارات `CREATE TABLE IF NOT EXISTS` فقط، ثم يشغّل خطوات
  `db/dataMigrations.js` بالترتيب — لذلك **إضافة عمود إلى جدول موجود لن تُطبَّق أبداً عبر
  `schema.sql` وحده**. عدّل `schema.sql` (للتنصيبات الجديدة) **و**أضف خطوة جديدة في
  `dataMigrations.js` (`ALTER TABLE` أو `UPDATE` صريح، آمنة للتكرار بذاتها) — لا تُعدّل خطوة
  قديمة موجودة، أضِف خطوة جديدة فقط
