...
بالتعديل بنفسه — والناشر وحده لاعتماد/رفض مناسبته. ⚠️ **التسليم الفعلي عبر Web Push محبوس
على دفعة لاحقة من #85** — `delivered_at` يبقى `NULL` لكل صف حتى ذلك الحين؛ الصف نفسه (ومن
يستحقّه ومتى يُنشأ) موجود ومكتمل من اليوم، ومركز الإشعارات هو سطح العرض الوحيد له حالياً.
...
`town_broadcast` أدناه. من لم يكن متصلاً لا يصله شيء حتى تسليم Web Push (دفعة لاحقة من #85)
— الصف نفسه في `GET /api/notifications` يبقى موجوداً بانتظاره. صفّ ينشئه المجدوِل اليومي
